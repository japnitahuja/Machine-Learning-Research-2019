...
for n0 in range(100):
	...
	for cluster in range(4):
		...
		cluster_kmeans = [[] for i in range(3)]
		for i in range(len(labels)):
			cluster_kmeans[labels[i]].append(i)
		sum_sse = 0
		for i in range(3):
			#print("recluster", i)
			for j in cluster_kmeans[i]:
				X_cluster = []
				X_cluster.append(X[i])

				X_centroid = [centroids_kmeans[i] for i in range(len(X_cluster))]
				#print(X[0])
				#print(X_centroid[0])
			X_cluster = np.array(X_cluster).astype(np.float64)
			X_centroid = np.array(X_centroid).astype(np.float64)
			#print(mse(X_cluster, X_centroid))
		sum_sse += sse(X_cluster, X_centroid)
		#print(sum_sse/len(X))
		result_temp.append(sum_sse/len(X))
	result_final.append(result_temp)
...
for n0 in range(3):
	...
	for i in range(4):
		...
		#print(i, index)
		vis[index] = 1
		temp.append(result_final[n0][index])
	#print(cluster_wise_acc[n0])
	#print(result_final[n0])
	#print(temp)
	result_out.append(temp)

#print(result_out)
with open("mse_recluster.txt", "w") as f:
	writer = csv.writer(f)
	writer.writerows(result_out)