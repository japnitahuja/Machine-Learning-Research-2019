...
op_file_path = os.path.join(path_metadata_folder,"expertise_space.txt")

#clear file
op_file = open(op_file_path,"w")
op_file.write("positive t value:decision tree is better, negative t value: " +
              "random forest is better, 0 t value: draw" + "\n")
op_file.close()
...
if os.path.exists(path_dataset_folder):
    ...
    for dataset in os.listdir(path_dataset_folder):
        ...
        accuracy_rf = [0 for i in range(100)]

        classifier_dt = tree.DecisionTreeClassifier()
        classifier_rf = sklearn.ensemble.RandomForestClassifier(n_estimators=64)
        ...
        for n0 in range(K):
            ...
            classifier_rf.fit(train_set_x,train_set_y)

            #accuracies
            accuracy_dt[n0] = classifier_dt.score(test_set_x,test_set_y)
            accuracy_rf[n0] = classifier_rf.score(test_set_x,test_set_y)

        x_coord = 0
        for i in accuracy_dt:
            x_coord += i
