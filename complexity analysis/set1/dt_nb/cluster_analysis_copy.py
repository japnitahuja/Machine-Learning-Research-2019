    ...
    for i in range(10):
        ...
        for j in range(3):
            if len(bins_es[i][j]) > 1:
                temp += str(round(np.mean(bins_es[i][j]),2)) + "," + str(round(np.var(bins_es[i][j]),2)) + "," + str(round(min(bins_es[i][j]),2)) + "," + str(round(max(bins_es[i][j]),2))
                temp += sep
            else:
                temp += "0" + sep
        if i == 0:
            ...
            temp += sep 
            temp += str(round(np.mean(baseline_measure),2)) + "," + str(round(np.var(baseline_measure),2)) + "," + str(round(min(baseline_measure),2)) + "," + str(round(max(baseline_measure),2))
            temp += sep    
            temp += "baseline accuracy"
            temp += sep
            temp += str(round(np.mean(baseline_accuracy),2)) + "," + str(round(np.var(baseline_accuracy),2)) + "," + str(round(min(baseline_accuracy),2)) + "," + str(round(max(baseline_accuracy),2))
            temp += sep
        temp = temp[:-1]
        temp += "\n"
        file_op.write(temp)
