...
for n0 in range(10):
    ...
    for test_fold in range(10):
        confusion_matrix_baseline = [[0 for i in range(3)] for j in range(3)]
        confusion_matrix_cluster = [[0 for i in range(3)] for j in range(3)]
        print(test_fold)
        ...
        for cluster in range(k_mean):
            ...
            #save the best k value for a specific cluster
            with open("best_cluster_k.txt", 'a') as f: 
                writer = csv.writer(f)
                temp = []
                temp.append(n0)
                temp.append(test_fold)
                temp.append(cluster)
                temp.append(best_cluster_k[cluster])
        ...
        #save the best k value for baseline dataset 
        with open("best_baseline_k.txt", "a") as f: 
            writer = csv.writer(f)
            temp = []
            temp.append(n0)
            temp.append(test_fold)
            temp.append(best_baseline_k)
            writer.writerow(temp)
        ...
        for cluster in range(4):
            ...
            new_prediction_baseline = []
            new_prediction_cluster = []
            for i in range(len(cluster_test)):
                confusion_matrix_baseline[int(y_test[i])][int(baseline_classifier.predict([x_test_baseline[i]]))] += 1
                confusion_matrix_cluster[int(y_test[i])][int(cluster_classifier.predict([x_test_cluster[i]]))] += 1
                new_prediction_baseline.append(baseline_classifier.predict([x_test_baseline[i]]))
                new_prediction_cluster.append(cluster_classifier.predict([x_test_cluster[i]]))
            ...
            with open("test_fold_baseline_prediction.txt", "a") as f: 
                writer = csv.writer(f)
                temp = []
                temp.append(n0)
                temp.append(test_fold)
                temp.append(cluster)
                writer.writerow(temp)
                writer.writerow(new_prediction_baseline)

            with open("test_fold_cluster_prediction.txt", "a") as f: 
                writer = csv.writer(f)
                temp = []
                temp.append(n0)
                temp.append(test_fold)
                temp.append(cluster)
                writer.writerow(temp)
                writer.writerow(new_prediction_cluster)
        ...
        with open("confusion_matrix_baseline.txt", "a") as f: 
            writer = csv.writer(f)
            temp = []
            temp = confusion_matrix_baseline[:]
            temp.append("\n")
            writer.writerows(temp)


        with open("confusion_matrix_cluster.txt", "a") as f: 
            writer = csv.writer(f)
            temp = []
            temp = confusion_matrix_baseline[:]
            temp.append("\n")
