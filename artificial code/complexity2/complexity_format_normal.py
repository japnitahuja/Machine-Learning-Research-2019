...
for i in range(len(measures["overlapping.F1"])):
    cnt = 0
    for j in measures.keys():
        if (measures[j][i] != "Inf" and measures[j][i] != "inf") and float(measures[j][i]) >= max_x1[cnt]:
            max_x1[cnt] = float(measures[j][i])
            #print("Inf", j,i)
        cnt += 1
...
for i in range(len(measures["overlapping.F1"])):
    cnt = 0
    for j in measures.keys():
        if measures[j][i] == "Inf" or measures[j][i] == "inf":
            measures[j][i] = max_x1[cnt]*2

        cnt += 1


result = [[None for i in range(22)] for j in range(400)]
for i in range(400):
    cnt = 0
    for j in measures.keys():
...
with open("complexity_cluster_format_normal.txt", "w") as f:
    writer = csv.writer(f)
